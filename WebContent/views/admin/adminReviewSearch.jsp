      ...
      <div id="main-div">
         ...
         <section id="tab-adminReview" class="tab-adminpage">
            ...
            <div id="search" align="right">작성자 검색&nbsp;
            	<input id="keyword" type="text" placeholder="작성자를 기입해주세요" size="15" value="<%= request.getAttribute("keyword")%>">
           ...
           <table id="adminCustomer" class="table-standard">
            ...
            <br><br>
              	...
              	<div class="pagingArea1" align="center">
                   	<%if(!list.isEmpty()){ %>
						<!-- 맨 처음으로 -->
						<button class="btn-standard" onclick="location.href='<%= request.getContextPath() %>/search.hos?currentPage=1&keyword=<%= request.getAttribute("keyword") %>'">&lt;&lt;</button>
					
						<!-- 이전 페이지로 -->
						<button class="btn-standard" onclick="location.href='<%= request.getContextPath() %>/search.hos?currentPage=<%=currentPage - 1 %>&keyword=<%= request.getAttribute("keyword") %>'" id="beforeBtn">&lt;</button>
						...
						<% for(int p = startPage; p <= endPage;p++){ %>
							...
							<%} else{ %>
								<button id="numBtn"  class="btn-standard" onclick="location.href='<%=request.getContextPath() %>/search.hos?currentPage=<%=p%>&keyword=<%= request.getAttribute("keyword") %>'"><%= p %></button>
							<%} %>
						<%} %>
						
						<!-- 다음 페이지로 -->
						<button id="afterBtn"  class="btn-standard" onclick="location.href='<%= request.getContextPath()%>/search.hos?currentPage=<%= currentPage +1%>&keyword=<%= request.getAttribute("keyword") %>'">&gt;</button>
						<script>
						...
						<!-- 맨 끝으로 -->
						<button class="btn-standard" onclick="location.href='<%=request.getContextPath()%>/search.hos?currentPage=<%=maxPage %>&keyword=<%= request.getAttribute("keyword") %>'">&gt;&gt;</button>
					<%} %>
              	</div>
